...
export class SyncProductsUseCaseImpl implements SyncProductsUseCase {
  ...
  constructor(
    @inject('ShopifyProductsInputPort') private inputPort: ShopifyProductsInputPort,
    @inject('ShopifyProductsOutputPort') private outputPort: ShopifyProductsOutputPort
  ) {}
  ...
  async *execute(): AsyncGenerator<void> {
    ...
    for await (const productsBatch of this.inputPort.fetchProductsInBatches()) {
      let products = productsBatch.map((product: ShopifyProductDTO) => this.toDomain(product));
  ...
  toDomain(shopifyProductDTO: ShopifyProductDTO): Product {
    return new Product(
      ...
      shopifyProductDTO.published_at ? new Date(shopifyProductDTO.published_at) : null, 
