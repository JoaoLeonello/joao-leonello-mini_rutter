...
export class GetOrdersUseCaseImpl implements GetOrdersUseCase {
  ...
  async execute(): Promise<Order[]> {
    ...
    let ordersDomain = orders.map((order: ShopifyOrder) =>
      this.toDomain(order),
    );

    return ordersDomain.map((order: Order) => {
      // First filter order fields
      let filteredOrder = this.filterFields(order, [
        "_id",
        "_platform_id",
        "_line_items",
      ]);
      ...
      if (filteredOrder.line_items && filteredOrder.line_items.length > 0) {
        ...
        filteredOrder.line_items.forEach((lineItem: any) => {
          ...
          let filteredLineItem = this.filterFields(lineItem, [
            "_product_id",
            "_quantity",
          ]);
          const productId = filteredLineItem.product_id
            ? filteredLineItem.product_id
            : null;
