  ...
  describe('fetchOrdersInBatches', () => {
    it('should fetch orders in batches until maxOrders is reached', async () => {
            const mockOrdersBatch: Partial<ShopifyOrderDTO>[] = [
                { id: 1, closed_at: null, created_at: '2024-01-01', total_price: '100.00' },
            ];
    ...
    it('should limit the number of orders fetched to maxOrders', async () => {
        const mockOrdersBatch: Partial<ShopifyOrderDTO>[] = Array.from({ length: 300 }, (_, i) => ({
            id: i + 1,
            closed_at: null,
      ...
      expect(totalOrdersFetched).toBeLessThanOrEqual(shopifyOrdersRepository['maxOrders']);
    });
