...
export class OrderRepository implements ShopifyOrdersOutputPort {
  ...
  async storeOrders(shopifyOrderBatch: Order[]): Promise<void> {
    ...
    for (let order of shopifyOrderBatch) {
      ...
      if (this.existingOrdersMap.has(order.platformId ?? "")) {
        let existingOrder: ShopifyOrder = this.existingOrdersMap.get(
          order.platformId ?? "",
        )!;
  ...
  private async processLineItems(
    ...
    for (let shopifyOrder of shopifyOrderBatch) {
      for (let lineItem of shopifyOrder.lineItems ?? []) {
        let existingLineItem = this.existingLineItemsMap.get(
          String(lineItem.platformId),
        );
        ...
        if (lineItem.productId === null) {
          logger.warn(
            `Skipping line item ${lineItem.platformId} as it has no linked product.`,
            { lineItem },
          );
          continue;
        }
        ...
        if (!existingLineItem) {
          ...
          let orderExistent =
            this.existingOrdersMap.get(shopifyOrder.platformId ?? "") ?? null;
  ...
  async setupMaps(shopifyOrderBatch: Order[]) {
    ...
    const existingProducts = await shopifyProductRepository.find({
      where: { platform_id: In(productIds) },
    });
    this.existingProductsMap = new Map<string, ShopifyProduct>(
      existingProducts.map((product: ShopifyProduct) => [
        String(product.platform_id || "null"),
        product,
      ]),
    );
  }
  ...
  async getOrders(): Promise<ShopifyOrder[]> {
    const test = await AppDataSource.getRepository(ShopifyOrder)
