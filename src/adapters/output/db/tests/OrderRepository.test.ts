  ...
  describe('OrderRepository', () => {
    ...
    describe('storeOrders', () => {
      it('should save new orders and line items', async () => {
        const orders: Order[] = [
          new Order('1', '1001', [], 'graphqlApiId', true, 'confirmation', true, new Date(), 'USD', '10', '15', '5', 'en', 'paid', 'Order1', 1, 'USD', new Date(), 'shopify', '10', '', false, '0', '10', '15', '5', 12345, new Date(), 123, 'token')
        ];
      ...
      it('should log a warning when a line item has no linked product', async () => {
        ...
        expect(loggerSpy).toHaveBeenCalledWith(expect.stringContaining('Skipping line item'));
    ...
    describe('setupMaps', () => {
      it('should setup existing orders, line items, and products maps', async () => {
        ...
        mockOrderRepository.find.mockResolvedValue([new ShopifyOrder('1', 1001)]);
        mockLineItemRepository.find.mockResolvedValue([new EntityLineItem('1', 2002, 'name', 'title', 'price', 'vendor', 1, null, null)]);
        mockProductRepository.find.mockResolvedValue([new ShopifyProduct('1', 1001, 'name', 'title', 'body_html', 'vendor', 'product_type', new Date(), new Date(), 'status')]);
